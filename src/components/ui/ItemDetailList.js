            ...
            return _.map(columnKeys, (colKeyContainer, colKeyIndex)=>{
                ...
                if (Array.isArray(value)) {
                    ...
                    if (_.any(value, function(v){ return typeof v === 'object' && v; }) && colKeyContainer.childKeys == null) {
                        const links = value.map((link, i) => (<span key={i}><a href={itemUtil.atId(link)} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover">{ link.display_title }</a></span>));
                                        ...
                                        { allKeys.map((k, j)=>{
                                            ...
                                            if (!renderedSubVal) {
                                                renderedSubVal = isAnItem(embeddedRow[k]) ?
                                                    <a href={itemUtil.atId(embeddedRow[k])} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover">{ itemUtil.getTitleStringFromContext(embeddedRow[k]) }</a>
                                                    :
                ...
                if (isAnItem(value)) {
                    return { 'value' : <a href={itemUtil.atId(value)} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover">{ value.display_title }</a>, 'key' : colKey };
                }
                                            ...
                                            [<td key="rowNumber">{ i + 1 }.</td>]
                                                .concat(row.map(function(colVal, j){
                                                    ...
                                                    if (colVal.key === '@id' && val.slice(0,1) === '/') {
                                                        val = <a href={val} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover">{ val }</a>;
                                                    }
                                                    if (val && typeof val === 'object' && !React.isValidElement(val) && !Array.isArray(val)) {
                                                        if (isAnItem(val)) {
                                                            val = <a href={itemUtil.atId(val)} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover">{val.display_title}</a>;
                                                            ...
                                                            val = <a href={attachmentHref} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover" target="_blank" rel="noreferrer noopener">{attachmentTitle}</a>;
                                                        } else {
                                                    ...
                                                    if (Array.isArray(val) && val.length > 0 && !_.all(val, React.isValidElement)) {
                                                        ...
                                                        val = _.map(val, function (v, i) {
                                                            let item = null;
                                                            if (isAnItem(v)) {
                                                                item = <a href={itemUtil.atId(v)} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover">{v.display_title}</a>;
    ...
    */
    static formValue(
        ...
        if (item === null){
            ...
            if (keyPrefix === '@id'){
                return <a key={item} href={item} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover" target={popLink ? "_blank" : null} rel="noreferrer noopener">{item}</a>;
            }
            ...
            if(item.charAt(0) === '/' && item.indexOf('@@download') > -1){
                ...
                return <a key={item} href={item} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover" target="_blank" download rel="noreferrer noopener">{ attach_title || item }</a>;
            } else if (item.charAt(0) === '/') {
                if (popLink) return <a key={item} href={item} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover" target="_blank" rel="noreferrer noopener">{ item }</a>;
                else return <a key={item} href={item} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover">{ item }</a>;
            } else {
                ...
                if (schemaPropertyFormat && ['uri','url'].indexOf(schemaPropertyFormat) > -1 && item.slice(0,4) === 'http') {
                    return <a key={item} href={item} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover" target="_blank" rel="noreferrer noopener">{ item }</a>;
                } else {
        ...
        'columnDefinitionMap' : {
            ...
            'subscriptions.url' : {
                'render' : function(value){
                    ...
                    return <a href={fullUrl} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover">View Results</a>;
                },
                'title' : "Link",
            ...
            'email' : {
                ...
                'render' : function(value){
                    if (typeof value === 'string' && value.indexOf('@') > -1) {
                        return <a href={'mailto:' + value} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover">{ value }</a>;
                    }
                    return value;
                }
            }
